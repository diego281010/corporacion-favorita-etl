...
def analisis_ventas_generales():
    ...
    for i, row in enumerate(bottom_10_tiendas.rows()):
        print(f"   {i + 1}. Tienda {row[0]}: ${row[1]:,.2f}")
    resultados["top_10_tiendas"] = ventas_por_tienda.head(10).to_dicts()
    resultados["bottom_10_tiendas"] = bottom_10_tiendas.to_dicts()
